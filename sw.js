...
// Automatically detect base path (works for both localhost and GitHub Pages)
const getBasePath = () => {
...
// Generate Bible data file URLs
const BIBLE_DATA = BIBLE_BOOKS_META.map(book => {
  const sanitizedName = book.name.replace(/\s/g, '');
  return `${BASE_PATH}/data/json/${sanitizedName}.json`;
});

// All URLs to cache
const CACHE_URLS = [...APP_SHELL, ...BIBLE_DATA];
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('[SW] Caching app shell and Bible data');
        return cache.addAll(CACHE_URLS);
      })
      .then(() => {
        console.log('[SW] Install complete - all resources cached');
      })
      .catch(err => {
